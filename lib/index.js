...
exports.Config = koishi_1.Schema.intersect([
    ...
    koishi_1.Schema.object({
        ...
        机器人账号: koishi_1.Schema.string().description('机器人子频道 id。').required(),
        指令转发QQ消息: koishi_1.Schema.boolean().default(false).description('是否指令触发。'),
        频道内触发指令: koishi_1.Schema.string().default('mc').description('QQ 内发送消息到 Minecraft 的指令，如有前缀请加上。'),
        指令转发MC消息: koishi_1.Schema.boolean().default(false).description('是否指令触发。'),
        游戏内触发指令: koishi_1.Schema.string().default('qq').description('Minecraft 内发送消息到 QQ 的指令，如有前缀请加上。')
    }).description('消息相关设置'),
    koishi_1.Schema.object({
    ...
    koishi_1.Schema.object({
        使用被动方式转发: koishi_1.Schema.boolean().default(false).description('当频道内有人发言时转发消息，配合触发时长使用。').experimental(),
        等待触发时长: koishi_1.Schema.number().default(2000).description('(毫秒) 时间段内可触发被动发送，超时采用主动发送。<br>启用后转发消息到 QQ 会有延迟，效果自行测试。').experimental(),
        使用备用频道: koishi_1.Schema.boolean().default(false).description('如果采用主动消息转发，在单个频道推送上限后向备用频道推送。').experimental(),
        备用转发频道: koishi_1.Schema.string().description('备用频道号').experimental(),
        启用定时任务: koishi_1.Schema.boolean().default(false).description('凌晨有一段时间无法推送。').experimental(),
        定时关闭转发频道消息: koishi_1.Schema.tuple([Number, Number]).default([0, 0]).description('(24小时制) 设置时，分。').experimental(),
        定时启动转发频道消息: koishi_1.Schema.tuple([Number, Number]).default([6, 0]).description('(24小时制) 设置时，分。').experimental()
    }).description('其他设置')
]);
function apply(ctx, config) {
    ...
    async function processWebSocketMessage(sendMessage) {
        try {
            if (max && max_) {
                if (!hasExecuted) {
            ...
            async function broadcastMessage(message_) {
                if (config.使用被动方式转发) {
    ...
    async function handleError(error, sendMessage) {
        // 主动推送上限，没遇到过，后续添加
